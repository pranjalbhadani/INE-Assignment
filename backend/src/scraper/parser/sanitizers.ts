...
export function sanitizePriceText(rawText: string): number | null {
  ...
  // Convert fullwidth digits to halfwidth
  let clean = rawText.replace(/[０-９]/g, (s) =>
    String.fromCharCode(s.charCodeAt(0) - 0xFEE0)
  );

  // Remove all variations of zero-width spaces
  clean = clean.replace(/[\u200B-\u200D\uFEFF]/g, '');

  // Handle compact prices such as ₹1.16k -> 1160
  const compactMatch = clean.match(/([\d.,]+)\s*k\b/i);

  if (compactMatch) {
    const value = parseFloat(compactMatch[1]!.replace(/,/g, ''));
  ...
  // Existing behavior for normal prices
  // ₹1,160 -> 1160
  // ₹1160 -> 1160
  // ₹999.50 -> 999.5
  clean = clean.replace(/[^\d.]/g, '');
